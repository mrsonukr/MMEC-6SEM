...
const iconMap = {
  like:    { icon: Heart,         bg: 'bg-red-950/40',     color: 'text-red-400' },
  comment: { icon: MessageCircle, bg: 'bg-blue-950/40',    color: 'text-blue-400' },
  follow:  { icon: UserPlus,      bg: 'bg-violet-950/40',  color: 'text-violet-400' },
  event:   { icon: CalendarDays,  bg: 'bg-purple-950/40',  color: 'text-purple-400' },
  job:     { icon: Briefcase,     bg: 'bg-emerald-950/40', color: 'text-emerald-400' },
          ...
          <div className="flex items-center justify-between mb-4 sm:mb-6 animate-fade-up">
            <div>
              <div className="flex items-center gap-2 mb-1">
                <Bell size={16} sm:size={18} className="text-violet-400" />
                <h1 className="font-display text-2xl sm:text-3xl font-semibold text-[#F9FAFB]">Notifications</h1>
                {unreadCount > 0 && (
                  <span className="btn-gold w-5 h-5 sm:w-6 sm:h-6 rounded-full flex items-center justify-center text-xs font-bold text-white">
                    {unreadCount}
                  </span>
                )}
              </div>
              <p className="text-xs sm:text-sm text-[#6B7280]">Stay up to date with your network</p>
            </div>
            {unreadCount > 0 && (
              <button
                onClick={markAllRead}
                className="flex items-center gap-1.5 text-xs text-violet-400 hover:text-[#F9FAFB] font-medium transition-colors whitespace-nowrap"
              >
                <Check size={13} /> <span className="hidden sm:inline">Mark all read</span><span className="sm:hidden">Read all</span>
              </button>
            )}
          </div>

          {/* Filter tabs */}
          <div className="flex gap-1.5 sm:gap-2 mb-4 sm:mb-6 overflow-x-auto no-scrollbar animate-fade-up stagger-1 pb-1">
            {filters.map(f => (
              <button key={f}
                onClick={() => setFilter(f)}
                className={`px-3 sm:px-4 py-1.5 sm:py-2 rounded-full text-xs font-medium whitespace-nowrap transition-all flex-shrink-0
                  ${filter === f
                    ? 'btn-gold text-white'
          ...
          <div className="space-y-2">
            {filtered.map((n, idx) => {
              ...
              return (
                <div
                  ...
                  className={`flex items-center gap-3 sm:gap-4 p-3 sm:p-4 rounded-2xl border transition-all cursor-pointer animate-fade-up
                    ${n.unread
                ...
                >
                  <div className="relative flex-shrink-0">
                    <img src={n.avatar} className="w-10 h-10 sm:w-11 sm:h-11 rounded-full object-cover ring-2 ring-violet-800/50 ring-offset-1" />
                    <div className={`absolute -bottom-1 -right-1 w-5 h-5 sm:w-6 sm:h-6 rounded-full ${bg} border-2 border-[#0A0A0A] flex items-center justify-center`}>
                      <Icon size={10} className={color} />
                    </div>
                  </div>

                  <div className="flex-1 min-w-0">
                    <p className="text-xs sm:text-sm text-[#F9FAFB] leading-snug">
                      <span className="font-semibold">{n.user}</span>{' '}
                      <span className="text-[#D1D5DB]">{n.action}</span>
                    </p>
                    <p className="text-[10px] sm:text-xs text-[#6B7280] mt-0.5">{n.time}</p>
                  </div>
            ...
            {filtered.length === 0 && (
              <div className="text-center py-16">
                <Bell size={32} className="text-violet-400/40 mx-auto mb-3" />
