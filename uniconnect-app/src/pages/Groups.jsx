    ...
    <div className="min-h-screen flex bg-[#0A0A0A] page-enter">
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="max-w-5xl mx-auto px-6 py-8">
          ...
          <div className="flex items-center justify-between mb-8 animate-fade-up">
            <div>
              <div className="flex items-center gap-2 mb-1">
                ...
                <h1 className="font-display text-3xl font-semibold text-[#F9FAFB]">Groups</h1>
              </div>
              <p className="text-sm text-[#6B7280]">Connect with communities that share your interests</p>
            </div>
          ...
          {/* Search */}
          <div className="relative mb-8 max-w-md animate-fade-up stagger-1">
          ...
          {joined.size > 0 && (
            <div className="mb-8 animate-fade-up stagger-2">
              <p className="text-xs text-[#6B7280] uppercase tracking-widest mb-3 font-medium">Your Groups</p>
              <div className="flex flex-wrap gap-2">
                {groups.filter(g => joined.has(g.id)).map(g => (
                  <span key={g.id}
                    className="px-4 py-1.5 rounded-full text-xs font-medium bg-violet-950/40 text-violet-400 border border-[#111]/[0.12]"
                  >
                    {g.name}
                  </span>
                ))}
            ...
            {filtered.map((group, idx) => (
              ...
              >
                ...
                <div className="p-5">
                  ...
                  <p className="text-xs text-[#D1D5DB] mb-4 leading-relaxed">{group.description}</p>
                  ...
                  <div className="flex items-center gap-2 mb-4">
                    <div className="flex -space-x-2">
                      {avatarSeeds.slice(0, 3).map((s, i) => (
                        <img key={i}
                          src={`https://i.pravatar.cc/100?img=${s}`}
                          className="w-7 h-7 rounded-full border-2 border-[color:var(--ivory)] object-cover"
                        />
                      ))}
                    </div>
