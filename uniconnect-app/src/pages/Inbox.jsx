...
import { Search, Send, MoreHorizontal, Phone, Video, Smile, Paperclip, ArrowLeft } from 'lucide-react'
...
export default function Inbox() {
  ...
  const ConversationList = (
    <div className="flex flex-col h-full bg-[#111111]">
      <div className="p-4 sm:p-5 border-b border-[#111]/[0.07]">
        ...
        {filtered.map((conv) => (
          ...
          >
            <div className="relative flex-shrink-0">
              ...
              {conv.online && <span className="absolute bottom-0.5 right-0.5 w-2.5 h-2.5 bg-emerald-400 rounded-full border-2 border-[#111]" />}
            </div>
  ...
  const ChatArea = activeConv && (
    <div className="flex flex-col h-full bg-[#0A0A0A]">
      {/* Chat header */}
      <div className="flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 bg-[#111111] border-b border-[#111]/[0.07]">
        <div className="flex items-center gap-3">
          {/* Back button on mobile */}
          <button
            onClick={() => setActiveConv(null)}
            className="md:hidden p-1.5 rounded-lg text-[#6B7280] hover:text-[#F9FAFB] transition-colors mr-1"
          >
            <ArrowLeft size={18} />
          </button>
          <div className="relative">
            <img src={activeConv.avatar} className="w-9 h-9 sm:w-10 sm:h-10 rounded-full object-cover ring-2 ring-violet-800/50" />
            {activeConv.online && <span className="absolute bottom-0 right-0 w-2.5 h-2.5 bg-emerald-400 rounded-full border-2 border-[#111]" />}
          </div>
          <div>
      ...
      {/* Messages */}
      <div className="flex-1 overflow-y-auto no-scrollbar px-4 sm:px-6 py-4 sm:py-5 space-y-3">
        {messages.map(msg => (
      ...
      {/* Input */}
      <div className="px-4 sm:px-6 py-3 sm:py-4 bg-[#111111] border-t border-[#111]/[0.07] pb-safe">
        <div className="flex items-center gap-2 sm:gap-3">
          <div className="flex gap-1">
            {[Paperclip, Smile].map((Icon, i) => (
              <button key={i} className="p-1.5 sm:p-2 rounded-lg text-[#6B7280] hover:text-violet-400 hover:bg-violet-950/40 transition-all">
                <Icon size={16} />
              </button>
            ))}
          </div>
          <input
            ...
            className="flex-1 bg-[#0A0A0A] border border-[#111]/[0.12] rounded-full px-4 py-2 sm:py-2.5 text-sm text-[#F9FAFB] focus:outline-none focus:border-violet-500 transition-all placeholder:text-[#6B7280]"
          />
          <button onClick={sendMessage} className="btn-gold p-2 sm:p-2.5 rounded-full flex-shrink-0">
  ...
  const EmptyState = (
    <div className="flex-1 flex items-center justify-center bg-[#0A0A0A]">
      <div className="text-center">
        <div className="w-16 h-16 rounded-full bg-violet-950/40 border border-violet-800/30 flex items-center justify-center mx-auto mb-4">
          <Send size={24} className="text-violet-400" />
        </div>
        <p className="text-sm font-medium text-[#F9FAFB] mb-1">Select a conversation</p>
        <p className="text-xs text-[#6B7280]">Choose from your messages on the left</p>
      </div>
    </div>
  )
    ...
    <div className="min-h-screen flex bg-[#0A0A0A] page-enter">
      ...
      <div className="flex-1 flex overflow-hidden" style={{ height: '100vh' }}>
        ...
        <div className={`
          ...
          pb-16 md:pb-0
        `}>
          {activeConv ? ChatArea : EmptyState}
        </div>
