...
import { PenLine, Hash, TrendingUp, Flame, Sparkles } from 'lucide-react'
...
const posts = [
  {
    ...
    time:'2 hours ago',
    text:'Hello everybody! We are preparing a new campaign for our upcoming design showcase. So excited to share it with the community 🎨',
  ...
  },
  {
    ...
    time:'5 hours ago',
    text:'Just wrapped up an amazing collab shoot in the mountains. The golden hour light was absolutely breathtaking 🏔️',
  ...
  },
  {
    ...
    time:'Yesterday',
    text:'New reel dropping tonight. Stay tuned! Working on something special for the frontend community 💻',
    image:''
  },
  {
    name:'Ryan Carter',
    ...
    time:'1 day ago',
    text:'Spent the weekend contributing to open source. Nothing beats solving real world bugs!',
  ...
  },
  {
    ...
    time:'2 days ago',
    text:'UI design tip: whitespace is not empty space, it is breathing room for your design.',
  ...
  return (
    <div className="min-h-screen flex bg-[#0A0A0A] page-enter">
      ...
      <div className="flex-1 flex justify-center overflow-x-hidden">
        <div className="w-full max-w-5xl flex gap-7 px-3 sm:px-6 py-4 sm:py-8 pb-24 md:pb-8">

          {/* Feed */}
          <div className="flex-1 min-w-0 space-y-4">

            {/* Compose */}
            <div className="card animate-fade-up p-3 sm:p-4">
              <div className="flex items-center gap-3 mb-3">
                <div className="relative flex-shrink-0">
                  <img
                    src="https://randomuser.me/api/portraits/men/75.jpg"
                    className="w-9 h-9 sm:w-10 sm:h-10 rounded-full ring-2 ring-violet-800/40 ring-offset-1 ring-offset-[#111] object-cover"
                  />
                ...
                <button
                  ...
                  className="flex-1 text-left text-sm text-[#4B5563] bg-white/[0.04] hover:bg-violet-950/30 border border-[#111]/[0.07] hover:border-violet-700/40 rounded-full px-4 py-2 transition-all duration-250"
                >
                ...
                <button
                  onClick={() => setShowCompose(s => !s)}
                  className="p-2 rounded-xl bg-violet-950/50 text-violet-400 hover:bg-violet-600 hover:text-white transition-all duration-200 hover:scale-110 active:scale-95 flex-shrink-0"
                >
                  <PenLine size={13} />
                </button>
              </div>
                ...
                <div className="animate-fade-up">
                  ...
                  <div className="flex justify-between items-center mt-3">
                    <div className="flex gap-1">
                      {['📷','🎬','📅','😊'].map(e => (
                        <button key={e} className="text-lg p-1.5 rounded-lg hover:bg-white/[0.05] transition-all hover:scale-110 active:scale-95">
                          {e}
                        </button>
                      ))}
                    </div>
      ...
      {/* Floating New Post Button — hidden on mobile (bottom nav handles it via compose) */}
      <button
        onClick={() => setShowCompose(true)}
        className="hidden md:flex fixed bottom-6 right-6 z-[9999] w-12 h-12 items-center justify-center rounded-full btn-gold shadow-xl hover:scale-110 transition"
      >
        <PenLine size={18} />
      </button>
